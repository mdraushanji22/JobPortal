...
<div class="max-w-4xl mx-auto px-4 py-8">
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6 mb-6">
        <div class="flex justify-between items-start mb-6">
            <div>
                <h1 class="text-3xl font-bold mb-2 text-gray-900 dark:text-white">{{ $job->title }}</h1>
        ...
        <div class="mb-6">
            <h3 class="text-lg font-semibold mb-2 text-gray-800 dark:text-gray-200">Responsibilities</h3>
            <p class="text-gray-700 dark:text-gray-300">{{ $job->responsibilities }}</p>
        </div>
        @endif

        @if($job->requirements)
        <div class="mb-6">
            <h3 class="text-lg font-semibold mb-2 text-gray-800 dark:text-gray-200">Requirements</h3>
        ...
        @if($job->benefits)
        <div class="mb-6">
            <h3 class="text-lg font-semibold mb-2 text-gray-800 dark:text-gray-200">Benefits</h3>
            <p class="text-gray-700 dark:text-gray-300">{{ $job->benefits }}</p>
        </div>
        @endif

        @if($job->skills_required)
        <div class="mb-6">
            <h3 class="text-lg font-semibold mb-2 text-gray-800 dark:text-gray-200">Skills Required</h3>
        ...
        <div class="flex flex-wrap gap-3">
            @auth
                @if(Auth::user()->isCandidate())
                    ...
                    @endif
                @else
                    <a href="{{ route('candidate.dashboard') }}" class="px-6 py-3 bg-blue-600 text-white rounded hover:bg-blue-700 transition"><i class="fas fa-paper-plane mr-2"></i>Switch to Candidate to Apply</a>
                @endif
            @else
                <a href="{{ route('login') }}" class="px-6 py-3 bg-blue-600 text-white rounded hover:bg-blue-700 transition"><i class="fas fa-paper-plane mr-2"></i>Login to Apply</a>
                <a href="{{ route('register') }}" class="px-6 py-3 border border-blue-600 text-blue-600 dark:text-blue-400 rounded hover:bg-blue-50 dark:hover:bg-gray-700 transition">Create Account</a>
            @endauth
        </div>
    </div>
</div>
