                ...
                <td class="px-6 py-4">
                    <div class="flex space-x-2">
                        ...
                        <form action="{{ route('candidate.applications.withdraw', $app) }}" method="POST" class="inline" onsubmit="return confirm('Withdraw application?')">
                            @csrf @method('DELETE')
                            <button type="submit" class="text-red-600 hover:text-red-800 text-sm">Withdraw</button>
                        </form>
                        @endif
                        @if($app->interview)
                            <span class="text-green-600 text-sm"><i class="fas fa-calendar-check mr-1"></i>Interview: {{ $app->interview->scheduled_date->format('M d') }}</span>
                        @endif
                    </div>
                </td>
            </tr>
