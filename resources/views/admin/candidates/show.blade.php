...
<div class="max-w-3xl mx-auto">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
        <div class="flex items-center mb-6">
            ...
            @else
                <div class="w-20 h-20 bg-gray-300 dark:bg-gray-600 rounded-full mr-4 flex items-center justify-center">
                    <i class="fas fa-user text-3xl text-gray-500 dark:text-gray-300"></i>
                </div>
            @endif
            <div>
                <h2 class="text-xl font-semibold dark:text-white">{{ $candidate->user->name }}</h2>
        ...
        <div class="grid grid-cols-2 gap-4 dark:text-gray-300">
            ...
            <div><strong>Portfolio:</strong> @if($candidate->portfolio_url) <a href="{{ $candidate->portfolio_url }}" target="_blank" class="text-blue-600 dark:text-blue-400">Link</a> @else N/A @endif</div>
            <div><strong>LinkedIn:</strong> @if($candidate->linkedin_url) <a href="{{ $candidate->linkedin_url }}" target="_blank" class="text-blue-600 dark:text-blue-400">Link</a> @else N/A @endif</div>
        </div>
        <div class="mt-6">
            <h3 class="font-semibold mb-2 dark:text-white">Applied Jobs</h3>
            @foreach($candidate->applications as $app)
                <div class="p-3 bg-gray-50 dark:bg-gray-700 rounded mb-2">
                    <p class="dark:text-gray-300">{{ $app->jobListing->title }} - <span class="text-sm text-gray-500 dark:text-gray-400">{{ $app->status }}</span></p>
                </div>
            @endforeach
        </div>
    </div>
